...
contract LiquidityGenerator is Ownable {
    ...
    IStarSwapRouter02 public starSwapRouter;
    ...
    uint public periodDuration = 30 minutes; // 3 days;    // Period to deposit STAR
    ...
    event Finalized(uint amountStarSwap, uint amountETH);
    event Deposit(
    ...
    constructor(
        ...
        // StarSwapRouter address in Astar mainnet
//      starSwapRouter = IStarSwapRouter02(0x16327e3fbdaca3bcf7e38f5af2599d2ddc33ae52);

        // StarSwapRouter address in Astar testnet
        starSwapRouter = IStarSwapRouter02(0xae50e11352F5D6B00d17b7222F402EA5b3DDbAfE);

        starSwapPair = IStarSwapFactory(starSwapRouter.factory())
            .getPair(star_, starSwapRouter.WETH());

        if (starSwapPair == address(0)) {
            starSwapPair = IStarSwapFactory(starSwapRouter.factory())
                .createPair(star_, starSwapRouter.WETH());
        }
    ...
    function deliverLiquidityToReservesManager() public onlyOwner {
        ...
        uint _amountPair = IERC20(starSwapPair).balanceOf(address(this));
        starSwapPair.safeTransfer(reservesManager, _amountPair);
    ...
    function finalize() public onlyOwner {
        ...
        uint _amountSTARInStarSwap = STAR.myBalance();
        uint _amountASTARInStarSwap = address(this).balance;

        STAR.safeApprove(address(starSwapRouter), _amountSTARInStarSwap);
        IStarSwapRouter01(starSwapRouter).addLiquidityETH{value: _amountASTARInStarSwap}(
            STAR,
            _amountSTARInStarSwap,
            0,
            0,
            address(this),
            blockTimestamp
        ...
        finalized = true;
        emit Finalized(_amountSTARInStarSwap, _amountASTARInStarSwap);
    }
    ...
    function deposit() external payable {
        ...
        require(msg.value >= 1e17, "LiquidityGenerator: INVALID_VALUE");        // minium is 0.1 STAR
        require(msg.value <= 1e22, "LiquidityGenerator: INVALID_VALUE");        // maxium is 10K STAR
    ...
    function withdrawASTAR() public onlyOwner {
        require(address(this).balance > 0, "LiquidityGenerator : No balance of STAR.");
        require(payable(msg.sender).send(address(this).balance));
    } 

    function withdrawSTAR() public onlyOwner {
        require(IERC20(STAR).balanceOf(address(this)) > 0, "LiquidityGenerator : No balance of STAR.");
        IERC20(STAR).transfer(msg.sender, IERC20(STAR).balanceOf(address(this)));
    } 
}