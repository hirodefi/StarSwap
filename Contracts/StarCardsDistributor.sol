...
contract StarCardsDistributor is Ownable, ReentrancyGuard {
    address private rewardToken;
    address public operatorAddress;

    address private constant starCardsNFT = address(0x6b3842Ce66c90200c5D3Fb1B35bfaF3817300c67);

    // Info of each reward.
    uint[] public _distributeRewards;

    mapping(uint=>uint) public _nextWithdrawRewards;
    mapping (address => uint) private _claimedReward;
    ...
    function sendDividendsforNFTHolders(uint _amount) external onlyOperatorOrOwner {
        uint length = ERC721Enumerable(starCardsNFT).totalSupply();
    ...
    function getRewardsOfTokenId(uint _tokenId) public view returns(uint) {
        uint nextWithdrawRewards = _nextWithdrawRewards[_tokenId];
    ...
    function getTotalRewardsOfNFT(address _account) public view returns(uint) {
        require(_account != address(0), "Wrong address");

        if (_excludedHolders[_account] == true )
            return 0;

        uint balance = ERC721Enumerable(starCardsNFT).balanceOf(_account);

        uint rewards;
        uint tokenId;
        for (uint i=0; i<balance; i++) {
            tokenId = ERC721Enumerable(starCardsNFT).tokenOfOwnerByIndex(_account, i);
            rewards += getRewardsOfTokenId(tokenId);
        }

        return rewards;
    }

    function withdrawn(address _account) public view returns (uint){
        require(_account != address(0), "Wrong address");

        return _claimedReward[_account];
    }
    ...
    function claim() external nonReentrant {
        ...
        uint totalRewards = getTotalRewardsOfNFT(msg.sender);
        
        require(totalRewards <= IERC20(rewardToken).balanceOf(address(this)), "Insufficient balance.");
        
        require(IERC20(rewardToken).transfer(msg.sender, totalRewards), "CANNOT GIVE REWARD!");

        _claimedReward[msg.sender] += totalRewards;

        emit Withdraw(msg.sender, totalRewards);

        updateNFTs(msg.sender);
    }

    function updateNFTs(address _account) internal {
        uint balance = ERC721Enumerable(starCardsNFT).balanceOf(_account);
        uint tokenId;
        for (uint i=0; i<balance; i++) {
            tokenId = ERC721Enumerable(starCardsNFT).tokenOfOwnerByIndex(_account, i);
            _nextWithdrawRewards[tokenId] = _distributeRewards.length;
        }
    }
    
    function setExcludedHolder(address _account, bool _exclude) public onlyOwner {
        _excludedHolders[_account] = _exclude;
    }
}