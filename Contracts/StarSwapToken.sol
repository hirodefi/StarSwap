...
interface StarSwapNFTStaking {
    function addRewards(uint _amount) external;
    function getOperator() external view returns (address);
}

interface StarCardsDistributor {
    function sendDividendsforNFTHolders(uint _amount) external;
    function getOperator() external view returns (address);
}
...
contract StarSwapToken is ERC20, Ownable {
    ...
    // starCardsDistributor contract address
    address public starCardsDistributorWallet = 0xFC242f42127494934efc79386ec03DCc8991d095;
    ...
    uint8 private _updateNFTStakingCount;
    ...
    // Parameters for NFT Staking
    address public nftStakingWallet;
    uint8 private nftMintCount;
    uint8 private MAXIMUM_MINT_NFTSTAKING = 10;
    uint256 private MAXIMUM_MINTAMOUNT_NFTSTAKING = 10_000_000 * 10 ** 18;          // 10M
    ...
    event StarSwapNFTStakingUpdated(address indexed operator, address indexed nftStaking);
    event StarSwapNFTStakingMinted(address indexed operator, address indexed nftStaking, uint256 mintAmount);
...
//  constructor() public ERC20("StarSwap Token", "STAR") {
    ...
    function setStarCardsRate(uint256 value) external onlyOperator{
        starCardsRate = value;
    }
    ...
    function setStarCardsDistributor(address _wallet) public onlyOperator {
        require(_wallet != address(0), "Update StarCardsDistributor: Wrong address.");
        starCardsDistributorWallet = _wallet;
    }

    function sendStarCards(uint256 _amount) private {
        super._transfer(address(this), starCardsDistributorWallet, _amount);

        if (StarCardsDistributor(starCardsDistributorWallet).getOperator() == address(this))
            StarCardsDistributor(starCardsDistributorWallet).sendDividendsforNFTHolders(_amount);
    }
    ...
    /**
     * @dev Update the NFT staking contract and mint STAR to NFT staking reserve.
     * Can only be called by the current operator.
     */
    function updateNFTStaking(address _toNFTStaking) external onlyOperator {
        require(_toNFTStaking != address(0), "Update NFT Staking: Wrong address.");
        
        _updateNFTStakingCount ++;
        
        require(_updateNFTStakingCount <= MAXIMUM_UPDATE_CONTRACT, "Update NFT Staking: too much updating NFT Staking.");
        
        nftStakingWallet = _toNFTStaking;

        emit StarSwapNFTStakingUpdated(msg.sender, nftStakingWallet);
    }

    function mintNFTStaking(uint256 _amount) external onlyOperator {
        require(nftStakingWallet != address(0), "NFT Staking: Wrong address.");
        require(nftMintCount < MAXIMUM_MINT_NFTSTAKING , "NFT Staking: too many mint count.");
        require(_amount <= MAXIMUM_MINTAMOUNT_NFTSTAKING , "NFT Staking: too many mint tokens.");
        
        _mint(nftStakingWallet, _amount);
        if (StarSwapNFTStaking(nftStakingWallet).getOperator() == address(this))
            StarSwapNFTStaking(nftStakingWallet).addRewards(_amount);

        nftMintCount ++;

        emit StarSwapNFTStakingMinted(msg.sender, nftStakingWallet, _amount);
    }
